...
key R package use is [**tmap**](https://cran.r-project.org/web/packages/tmap/) package in R. Beside **tmap** package, four other R packages will be used which are listed as below:
...
1. Master Plan 2014 Subzone Boundary (Web) (i.e. MP14_SUBZONE_WEB_PL) in ESRI shapefile format.
- It can be downloaded at data.gov.sg and is a geospatial data. 
- Data consists of the geographical boundary of Singapore at the planning subzone level. 
- The data is based on URA Master Plan 2014.

2. Singapore Residents by Planning Area / Subzone, Age Group, Sex and Type of Dwelling, June 2011-2020 in csv format.(i.e. respopagesextod2011to2020.csv)
- This is an aspatial data fie and can be downloaded at Department of Statistics,Singapore 
...
```{r}
mpsz <- st_read(dsn = "C:/ythsieh14/ISSS626-GAA/Hands-on-Ex/Hands_on_Ex01-2/data/geospatial/MasterPlan2014SubzoneBoundaryWebSHP", layer = "MP14_SUBZONE_WEB_PL")
```
...
```{r}
popdata <- read_csv("C:/ythsieh14/ISSS626-GAA/Hands-on-Ex/Hands_on_Ex01-2/data/aspatial/respopagesextod2011to2020.csv")
```
Data Preparation
...
1. Before we can perform the georelational join, one extra step is required to convert the values in PA and SZ fields to uppercase.
  ...
  2. The SUBZONE_N and PLN_AREA_N are in uppercase.
...
Noted:left_join() of dplyr package is used with mpsz simple feature data frame as the left data table is to ensure that the output will be a simple features data frame.
...
qtm() is the Easiest and quickest to draw a choropleth map using tmap and it is also concise and provides a good default visualisation in many cases.
...
1. m_polygons() is a wraper of tm_fill() and tm_border().

2. tm_fill() shades the polygons by using the default colour scheme

3. tm_borders() adds the borders of the shapefile onto the choropleth map.
...
- Plotting choropleth maps with different built-in classification methods
...
tm_shape(mpsz_pop2020)+
  ...
  tm_borders(alpha = 0.5)
```
- Similar classification method but with different numbers of classes 
